...
class Config:
    ...
    # Puertos disponibles
    PORTS = {"debug": 8054, "produccion": 8055, "test": 8056}
    ...
    def clear_old_logs(hours=24):
        ...
        try:
                ...
                for line in f:
                    if line.strip():
                        try:
                            ...
                            new_logs.append(
                                line
                            )  # Conservar líneas con formato inválido
            ...
            LogManager.log_event(
                "MAINTENANCE", f"Logs antiguos borrados, conservados últimas {hours}h"
            )
...
class ClickDebugger:
    ...
    def analyze_click_chain():
        ...
        try:
            ...
            try:
                from Modulos.Analisis_modelos.plot_interactive import (
            ...
            results["components"]["customdata"] = ClickDebugger._analyze_customdata(
                fig_test
            )
            ...
            if "error" in component_statuses:
                ...
                results["message"] = (
                    "Warnings detectados, sistema funcional con optimizaciones posibles"
                )
    ...
    def _analyze_graph_id_architecture():
        ...
        try:
            ...
            main_viz_path = os.path.join(
                os.path.dirname(__file__), "main_visualizacion_modelos.py"
            )
            if os.path.exists(main_viz_path):
                with open(main_viz_path, "r", encoding="utf-8") as f:
                    ...
                    graph_patterns = re.findall(
                        r"dcc\.Graph\([^)]*id=['\"]([^'\"]*)['\"]", content
                    )

                    if graph_patterns:
                        unique_graph_ids = set(graph_patterns)
                        if (
                            len(unique_graph_ids) == 1
                            and "plot-graph" in unique_graph_ids
                        ):
                            print(
                                "   ✅ Arquitectura de IDs CORRECTA: Todos los gráficos usan 'plot-graph'"
                            )
                        elif len(unique_graph_ids) > 1:
                            issues_found.append(
                                f"Múltiples IDs de gráfico detectados: {unique_graph_ids}"
                            )
                            print(
                                f"   ❌ PROBLEMA CRÍTICO: Múltiples IDs detectados: {unique_graph_ids}"
                            )
                        else:
                            warnings_found.append(
                                f"ID no estándar encontrado: {unique_graph_ids}"
                            )

                    # Verificar que callbacks usan el ID correcto
                    callback_patterns = re.findall(
                        r"Input\(['\"]([^'\"]*)['\"],\s*['\"]clickData['\"]", content
                    )
                    if callback_patterns:
                        unique_callback_ids = set(callback_patterns)
                        if "plot-graph" in unique_callback_ids:
                            print(
                                "   ✅ Callbacks de clickData configurados correctamente"
                            )
                        else:
                            issues_found.append(
                                f"Callbacks no usan 'plot-graph': {unique_callback_ids}"
                            )
                            print(
                                f"   ❌ PROBLEMA: Callbacks usan IDs incorrectos: {unique_callback_ids}"
                            )
    ...
    def _verify_id_consistency():
        ...
        try:
            ...
            recommendations = ClickDebugger._generate_recommendations(
                results["components"]
            )
            ...
            component_statuses = [
                comp.get("status", "unknown") for comp in results["components"].values()
            ]
            if "error" in component_statuses:
                results["status"] = "error"
            elif "warning" in component_statuses:
    ...
    def _analyze_customdata(fig):
        ...
        for i, trace in enumerate(fig.data):
            ...
            if hasattr(trace, "customdata") and trace.customdata is not None:
                trace_info["customdata_shape"] = getattr(
                    trace.customdata, "shape", "N/A"
                )
                trace_info["customdata_sample"] = str(
                    trace.customdata[0] if len(trace.customdata) > 0 else "Empty"
                )
        ...
        return {
            ...
            "fix": (
                "Verificar que create_interactive_plot esté generando customdata correctamente"
                if status == "error"
                else None
            ),
        }
    ...
    def _analyze_callbacks():
        ...
        try:
            ...
            return {
                ...
                "fix": (
                    "Verificar que los callbacks de clickData estén correctamente definidos"
                    if not click_callbacks
                    else None
                ),
            }
        ...
        if "critical_checks" in results:
            ...
            if "id_consistency" in results["critical_checks"]:
                ...
                if consistency["status"] != "ok":
                    recommendations.append(
                        "🔄 Verificar sincronización entre layout y callbacks"
                    )
    ...
    def create_real_time_click_monitor():
        ...
        try:
            ...
            # Contador de clicks
            counter_label = widgets.HTML(value="<b>Clicks detectados:</b> 0")
            ...
            def toggle_monitor(button):
                if not monitor_state["active"]:
                    ...
                    monitor_thread["thread"] = threading.Thread(
                        target=monitor_click_detection, daemon=True
                    )
                    ...
                    with log_output:
                        print(f"\n🛑 Monitor detenido")
                        print(
                            f"📊 Total de clicks detectados: {monitor_state['click_count']}"
                        )
            ...
            monitor_widget = widgets.VBox(
                [title_widget, status_label, info_label, controls, log_output]
            )
    ...
    def toggle_console_debug(enable=True):
        ...
        if enable:
            ...
            print(
                "💡 Los clicks mostrarán información detallada en la consola cuando uses la aplicación"
            )
        else:
...
def launch_app_with_config(config):
    ...
    try:
        ...
        print(
            f"🐛 Debug: {'Activado' if config.get('debug', False) else 'Desactivado'}"
        )
        # Asegurar uso del JSON actual
        cfg = dict(config)
        cfg["json_path"] = Config.JSON_PATH
        print(f"📄 Usando JSON: {cfg.get('json_path', Config.JSON_PATH)}")

        main_visualizacion_modelos(
            json_path=cfg.get("json_path"),
            use_dash=cfg.get("use_dash", True),
...
def create_click_monitor_dashboard():
    ...
    try:
        ...
        monitor_app.layout = html.Div(
            [
                html.H1(
                    "🖱️ Monitor de Clicks en Tiempo Real",
        ...
        click_data = {"clicks": [], "timestamps": [], "total_clicks": 0}
        ...
        def update_monitor(n):
            ...
            stats = html.Div(
                [
                    ...
                    html.P(
                        f"Último Click: {click_data['timestamps'][-1].strftime('%H:%M:%S') if click_data['timestamps'] else 'N/A'}"
                    ),
                ]
            )
            ...
            recent = html.Div(
                [
                    ...
                    for click, ts in zip(
                        click_data["clicks"][-5:], click_data["timestamps"][-5:]
                    )
                ]
            )
...
class DiagnosticManager:
    ...
    def check_dependencies():
        ...
        for dep in Config.REQUIRED_DEPS:
            try:
                ...
                results[dep] = {
                    "status": "error",
                    "message": f"INSTALAR: pip install {dep}",
                }
                all_ok = False
    ...
    def analyze_dash_app():
        ...
        try:
            ...
            return {"status": "error", "message": f"Error analizando Dash: {str(e)}"}
        ...
        try:
            ...
            if app_analysis["status"] == "error":
                ...
                if isinstance(details, dict):
                    ...
                    click_count = (
                        len(click_callbacks) if isinstance(click_callbacks, list) else 0
                    )
            ...
            return {"status": "error", "message": f"Error testing clicks: {str(e)}"}
...
class MetricsMonitor:
    ...
    def record_callback(self, success=True):
        ...
        LogManager.log_event(
            "CALLBACK", f'Callback {"exitoso" if success else "fallido"}'
        )
    ...
    def launch_app(mode="debug"):
        ...
        try:
            ...
            if mode not in Config.MODES:
                raise ValueError(
                    f"Modo inválido: {mode}. Opciones: {list(Config.MODES.keys())}"
                )
            ...
            # Tomar copia y actualizar json_path con el valor ACTUAL de Config.JSON_PATH
            config = dict(Config.MODES[mode])
            ...
            failed_modules = [
                m for m, r in modules_result.items() if r["status"] == "error"
            ]
            if failed_modules:
            ...
            try:
                from Modulos.Analisis_modelos.main_visualizacion_modelos import (
                    main_visualizacion_modelos,
                )
            except ImportError:
                sys.path.append("Modulos")
                from Analisis_modelos.main_visualizacion_modelos import (
                    main_visualizacion_modelos,
                )

            # Configurar variables globales necesarias (siempre que existan datos)
            if isinstance(data_result, dict):
                globals()["modelos_por_celda"] = data_result.get("modelos_por_celda")
                globals()["detalles_por_celda"] = data_result.get("detalles_por_celda")
                globals()["total_modelos"] = data_result.get("total_modelos")

            # Lanzar aplicación
            print(f"🚀 Lanzando aplicación en modo {mode.upper()}...")
            print(f"🌐 URL: http://localhost:{config['port']}")
            print(f"📄 Usando JSON: {config.get('json_path', Config.JSON_PATH)}")
            ...
            LogManager.log_event(
                "LAUNCH",
                f"Aplicación lanzada exitosamente en modo {mode}",
                {
                    "port": config["port"],
...
class ControlPanel:
    ...
    def create_dashboard():
        ...
        output_json_debug = widgets.Output(
            layout=widgets.Layout(
                height="350px",
                width="100%",
                overflow="auto",
        ...
        def debug_json(button):
            with output_json_debug:
                ...
                try:
                    ...
                    print(
                        f"DF completos encontrados: {report['df_completos_encontrados']}"
                    )
                    print(
        ...
        def launch_debug_app(button):
            ...
            with output_main:
                ...
                try:
                    ControlPanel._show_centered_title(
                        "🐛 LANZANDO APLICACIÓN EN MODO DEBUG"
                    )
        ...
        def launch_production_app(button):
            ...
            with output_main:
                ...
                try:
                    ControlPanel._show_centered_title(
                        "🚀 LANZANDO APLICACIÓN EN MODO PRODUCCIÓN"
                    )
                    print("🔄 Verificando sistema...")
        ...
        launch_section = widgets.VBox(
            ...
            """
                ...
                widgets.HBox(
                    [debug_button, production_button],
        ...
        tools_section = widgets.VBox(
            ...
            """
                ...
                widgets.HBox(
                    [status_button, diagnostic_button, json_debug_button],
                    layout=widgets.Layout(justify_content="center"),
                ),
                widgets.HBox(
                    [click_debug_button, clear_logs_button],
                    layout=widgets.Layout(justify_content="center"),
                ),
            ]
        )
        ...
        main_panel = widgets.VBox(
            [
                ...
                results_section,
                widgets.HTML(
                    value="<h4 style='color:#d9534f; margin-top:20px;'>🐞 Debug JSON Output</h4>"
                ),
                output_json_debug,
            ],
        ...
        with output_main:
            ...
            print(
                "   • 🐞 Debug JSON: Validación y diagnóstico del archivo JSON de modelos"
            )
            print("   • 🗑️ Borrar Logs: Gestión de archivos de historial")
            print()
            print(
                "📌 Toda la salida se mostrará en este panel único con scroll automático"
            )
    ...
    def _display_status():
        ...
        print(
            f"📦 Dependencias: {'✅' if deps_ok else '❌'} ({sum(1 for d in deps.values() if d['status'] == 'ok')}/{len(deps)})"
        )
        ...
        if data_result:
            print(
                f"🗄️  Datos: ✅ {data_result['total_modelos']} modelos en {data_result['num_celdas']} celdas"
            )
        else:
        ...
        print(
            f"🧩 Módulos: {'✅' if modules_ok else '❌'} ({sum(1 for m in modules.values() if m['status'] == 'ok')}/{len(modules)})"
        )
        ...
        print(
            f"🖱️  Click Test: {'✅' if click_test['status'] == 'ok' else '⚠️' if click_test['status'] == 'warning' else '❌'} {click_test['message']}"
        )
        ...
        if "metrics_monitor" in globals():
            ...
            if metrics["app_start_time"]:
                print(
                    f"📈 App Activa: ✅ {metrics.get('uptime', 'N/A')} | Clicks: {metrics['clicks_received']} | Callbacks: {metrics['callbacks_executed']}"
                )
    ...
    def _display_status_detailed():
        ...
        deps, deps_ok = DiagnosticManager.check_dependencies()
        print(
            f"📦 DEPENDENCIAS ({sum(1 for d in deps.values() if d['status'] == 'ok')}/{len(deps)}):"
        )
        ...
        if data_result:
            print(
                f"   ✅ {data_result['total_modelos']} modelos en {data_result['num_celdas']} celdas"
            )
        ...
        modules = DiagnosticManager.check_modules()
        print(
            f"🧩 MÓDULOS ({sum(1 for m in modules.values() if m['status'] == 'ok')}/{len(modules)}):"
        )
        ...
        print(f"\n🖱️ TEST DE CLICKS:")
        print(
            f"   {status_emoji.get(click_test['status'], '❓')} {click_test['message']}"
        )
    ...
    def _display_status_detailed_improved():
        ...
        try:
            # Dependencias
            deps, deps_ok = DiagnosticManager.check_dependencies()
            ControlPanel._show_section_header(
                "📦 DEPENDENCIAS", "ok" if deps_ok else "error"
            )
            ...
            data_result, error = DataManager.load_models_data()
            ControlPanel._show_section_header(
                "🗄️ DATOS", "ok" if data_result else "error"
            )
            if data_result:
            ...
            modules_ok = all(m["status"] == "ok" for m in modules.values())
            ControlPanel._show_section_header(
                "🧩 MÓDULOS", "ok" if modules_ok else "error"
            )
            for module, result in modules.items():
                status_emoji = "✅" if result["status"] == "ok" else "❌"
                short_name = result.get("short_name", module.split(".")[-1])
            ...
            for name, port_info in ports.items():
                if port_info["occupied"]:
                    print(
                        f"   🟢 {name.title():<12} Puerto {port_info['port']} (ACTIVO)"
                    )
                else:
                    print(
                        f"   ⚪ {name.title():<12} Puerto {port_info['port']} (disponible)"
                    )
    ...
    def _run_diagnostic_improved():
        ...
        try:
            ...
            for name, port_info in ports.items():
                ...
                print(
                    f"   {status} {name.title():<12} Puerto {port_info['port']} ({estado})"
                )
            ...
            status = {"ok": "✅", "warning": "⚠️", "error": "❌"}[
                dash_analysis["status"]
            ]
            print(f"   {status} Dash App: {dash_analysis['message']}")

            if "details" in dash_analysis and isinstance(
                dash_analysis["details"], dict
            ):
                ...
                print(
                    f"   ⚙️  Callbacks totales: {details.get('callbacks_count', 'N/A')}"
                )
            ...
            print()
            final_status = (
                "✅ SISTEMA LISTO" if deps_ok and data_result else "❌ CORREGIR ERRORES"
            )
    ...
    def _display_ids_improved(ids_list, severity):
        ...
        severity_config = {
            "error": {
                "emoji": "🔴",
                "context": "Crítico - Puede causar fallos en callbacks",
            },
            "warning": {
                "emoji": "🟡",
                "context": "Advertencia - No afecta funcionalidad pero puede optimizarse",
            },
    ...
    def _run_click_debug_improved():
        ...
        try:
            ...
            ControlPanel._show_section_header(
                "� DIAGNÓSTICO DETALLADO DEL FLUJO DE CLICKS", "info"
            )
            ...
            if dash_analysis["status"] == "ok":
                ...
                if isinstance(details, dict):
                    ...
                    if details.get("missing_layout_ids") or details.get(
                        "orphaned_layout_ids"
                    ):
                        print(
                            f"   ⚠️  IDs inconsistentes detectados - pueden afectar clicks"
                        )
                    else:
                        print(f"   ✅ Todos los IDs están correctamente vinculados")
                else:
                    print(
                        f"   ⚠️  Detalles de Dash analysis no disponibles o malformados: {details}"
                    )
            else:
            ...
            try:
                ...
                if app and hasattr(app, "callback_map"):
                    ...
                    if click_callbacks:
                        print(
                            f"   ✅ {len(click_callbacks)} callbacks de click encontrados:"
                        )
            ...
            ControlPanel._show_section_header(
                "📡 MONITOR DE CLICKS EN TIEMPO REAL", "info"
            )
            ...
            try:
                ...
                if monitor_widget:
                    ...
                    print(
                        "   🔇 NO genera logs constantes - Solo detecta clicks reales"
                    )
                ...
                for comp_name, comp_data in analysis["components"].items():
                    ...
                    if isinstance(comp_data, dict):
                        if "traces_with_customdata" in comp_data:
                            print(
                                f"   📈 Traces con customdata: {len(comp_data['traces_with_customdata'])}"
                            )
                        if "traces_without_customdata" in comp_data:
                            print(
                                f"   ⚠️  Traces sin customdata: {len(comp_data['traces_without_customdata'])}"
                            )
                        if "click_callbacks" in comp_data:
                            print(
                                f"   🖱️ Click callbacks: {len(comp_data['click_callbacks'])}"
                            )
            else:
            ...
            if analysis["recommendations"]:
                ControlPanel._show_section_header(
                    f"💡 RECOMENDACIONES ESPECÍFICAS", "info"
                )
                ...
                print(
                    f"   {len(analysis['recommendations']) + 1}. Verificar que customdata esté presente en todos los traces"
                )
                print(
                    f"   {len(analysis['recommendations']) + 2}. Confirmar que callbacks de clickData estén correctamente registrados"
                )
                print(
                    f"   {len(analysis['recommendations']) + 3}. Usar el monitor en tiempo real para detectar si se generan eventos"
                )
                print()
            ...
            print(
                "   ℹ️  Warnings no críticos pueden optimizarse pero no afectan funcionalidad"
            )
