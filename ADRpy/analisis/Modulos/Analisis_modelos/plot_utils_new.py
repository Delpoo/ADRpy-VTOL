...
def add_normalized_model_curves(fig: go.Figure, 
    ...
    for i, modelo in enumerate(modelos):
        ...
        # Crear rango de valores X originales para la curva
    ...
    # Añadir advertencias al gráfico si las hay
    if warnings_added:
        warning_text = "Advertencias:<br>" + "<br>".join(warnings_added[:3])  # Limitar a 3 advertencias
        if len(warnings_added) > 3:
            warning_text += f"<br>... y {len(warnings_added) - 3} más"
...
def get_model_predictions_safe(modelo: Dict, x_range: np.ndarray) -> Optional[np.ndarray]:
    ...
    try:
        ...
        logger.error(f"Error generando predicciones para modelo tipo {tipo}: {e}")
        return None
