...
def calcular_correlaciones_y_generar_heatmap_con_resumen(df_procesado, parametros_seleccionados, valor_por_defecto=0.7):
    """
    ...
    try:
        # === Paso 1: Solicitar umbral al usuario ===
        ...
        parametros_no_encontrados = [v for v in parametros_seleccionados if v not in df_procesado.index]
        if parametros_no_encontrados:
            raise ValueError(f"Los siguientes parámetros no se encontraron en los datos procesados: {', '.join(parametros_no_encontrados)}")

        # === Tabla completa (sin filtrar) ===
        print("\n=== Cálculo de tabla completa ===")
        tabla_completa = df_procesado.transpose().corr()
        agregar_resumen_a_tabla(tabla_completa.round(3), "Tabla de Correlaciones con todos los parametros(tabla_completa)")
        ...
        heatmap_data = datos_filtrados.dropna(thresh=2)  # Excluir variables con menos de 2 valores válidos
        ...
        plt.title(f"Heatmap de Correlaciones de Variables Seleccionadas (Umbral >= {umbral})")
        plt.show()

    except ValueError as e:
        print(f"Error: {e}. Por favor verifica los parámetros seleccionados.")
    except KeyError as e:
        print(f"Error: {e}. Asegúrate de que las variables seleccionadas existen en los datos.")
