def seleccionar_parametros_por_indices(
    parametros_disponibles, parametros_preseleccionados, entrada_indices=None
):
...
def solicitar_umbral(valor_por_defecto=0.7, umbral_manual=None):
    ...
    if umbral_manual is not None:
        if not (0 < umbral_manual < 1):
            print(
                f"⚠️ El umbral_manual ({umbral_manual}) está fuera de rango. Se usará el valor por defecto ({valor_por_defecto})"
            )
    ...
    try:
        ...
        print(
            f"Valor inválido: {e}. Se usará el umbral por defecto de {valor_por_defecto}."
        )
        return valor_por_defecto
...
# ----- Opcional: Toggles para manejo de outliers -----
def solicitar_manejar_outliers(defecto: bool = True, valor: str | None = None) -> bool:
    try:
        if valor is None:
            entrada = input(
                f"¿Aplicar pesos robustos a outliers? (s/n, predeterminado {'s' if defecto else 'n'}): "
            )
        else:
            entrada = valor
        entrada = (entrada or ("s" if defecto else "n")).strip().lower()
        return entrada in ("s", "si", "sí", "y", "yes", "true", "1")
    except Exception:
        return defecto


def solicitar_umbral_z_suave(defecto: float = 3.0, valor: str | None = None) -> float:
    try:
        if valor is None:
            entrada = input(f"Umbral z suave (float, predeterminado {defecto}): ")
        else:
            entrada = valor
        return float(entrada) if entrada else defecto
    except Exception:
        return defecto


def solicitar_umbral_z_duro(defecto: float = 6.0, valor: str | None = None) -> float:
    try:
        if valor is None:
            entrada = input(f"Umbral z duro (float, predeterminado {defecto}): ")
        else:
            entrada = valor
        return float(entrada) if entrada else defecto
    except Exception:
...
def solicitar_remover_duro(defecto: bool = False, valor: str | None = None) -> bool:
    try:
        if valor is None:
            entrada = input(
                f"¿Excluir outliers duros? (s/n, predeterminado {'s' if defecto else 'n'}): "
            )
        else:
            entrada = valor
        entrada = (entrada or ("s" if defecto else "n")).strip().lower()
